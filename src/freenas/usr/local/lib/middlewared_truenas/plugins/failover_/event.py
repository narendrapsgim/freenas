...
class FailoverService(Service):
    ...
    def _event(self, ifname, vhid, event):
        ...
        # If there is no pool configured and we receive an
        # event for failover, we exit since there is nothing
        # to import.
        if not fobj['volumes'] + fobj['phrasedvolumes']:
            self.logger.warning(
                f'Failover event received for:{ifname} with vhid:{vhid} for event:{event}.'
                ' However, no zpools are configured so ignoring.'
            )
            return
